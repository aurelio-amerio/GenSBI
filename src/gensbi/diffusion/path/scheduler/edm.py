...
class BaseSDE(abc.ABC):
    ...
    def sample_noise(self, key
        Array, shape: Any, sigma: Array) -> Array:
        """
    ...
    def f(self, x
        ...
        Computes the drift term
            math:`f(x, t) = x \frac{ds}{dt} / s(t)` as used in the SDE formulation.
    ...
    def g(self, x
        ...
        Computes the diffusion term
            math:`g(x, t) = s(t) \sqrt{2 \frac{d\sigma}{dt} \sigma(t)}` as used in the SDE formulation.
    ...
    def denoise(self, F
        Callable, x: Array, sigma: Array, *args, **kwargs) -> Array:
        r"""
        Denoise function,
            math:`D` in the EDM paper, which shares a connection with the score function:

        .. math
            :
            \nabla_x \log p(x; \sigma) = \frac{D(x; \sigma) - x}{\sigma^2}

        This function includes the preconditioning and is connected to the NN objective
            math:`F`:
    ...
    # FIXME
        for some reason, when sampling using the score, instead of the algorithm provided by the EDM paper, the sample quality is very bad
    ...
    #     where
        math:`D(x; \sigma)` is the denoised output (see `denoise` method).
    ...
    #     """
    #     def score(x
        Array, u: Array, *args, **kwargs) -> Array:
    ...
    def get_loss_fn(self) -> Callable
        ...
        def loss_fn(
            F
                Callable, batch: tuple, loss_mask: Any = None, model_extras: dict = {}
        ) -> Array
            (x_1, x_t, sigma) = batch
            ...
            return jnp.mean(jnp.sum(loss, axis=tuple(range(1, len(x_1.shape)))))  # type
                ignore
...
class VPScheduler(BaseSDE)
    ...
    References
        - Karras, Tero, et al. "Elucidating the design space of diffusion-based generative models." `arXiv
            2206.00364 <https://arxiv.org/abs/2206.00364>`_
    """
