...
def test_log_prob_shape_and_finiteness():
    flow = MAFlow(MAFlowParams(rngs=nnx.Rngs(0), dim=3, cond_dim=2, n_layers=3,
...
def test_sample_shape_and_roundtrip_consistency():
    flow = MAFlow(MAFlowParams(rngs=nnx.Rngs(0), dim=3, cond_dim=2, n_layers=2,
...
def test_density_integrates_to_one_1d():
    ...
    flow = MAFlow(MAFlowParams(rngs=nnx.Rngs(0), dim=1, cond_dim=1, n_layers=3,
                               nn_width=32, nn_depth=2))
    cond = jnp.zeros((1,))
...
def test_set_standardization_raises_without_bijection():
    ...
    flow = MAFlow(MAFlowParams(rngs=nnx.Rngs(0), dim=2, cond_dim=1, n_layers=2,
                               standardize=False))
...
def test_zero_init_spline_flow_is_standard_normal():
    ...
    dim, cond_dim = 3, 2
    flow = MAFlow(MAFlowParams(rngs=nnx.Rngs(0), dim=dim, cond_dim=cond_dim,
                               n_layers=4,
