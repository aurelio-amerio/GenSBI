...
def test_name(scheduler_cls):
    ...
    assert isinstance(
        scheduler.name, str
    ), f"scheduler.name is not a string, got type {type(scheduler.name)} and value {scheduler.name}"
...
def test_sigma_and_inv(scheduler_cls):
    ...
    assert (
        t_inv.shape == t.shape
    ), f"sigma_inv: expected shape {t.shape}, got {t_inv.shape}"
...
def test_sigma_deriv(scheduler_cls):
    ...
    assert (
        deriv.shape == t.shape
    ), f"sigma_deriv: expected shape {t.shape}, got {deriv.shape}"
...
def test_s_and_deriv(scheduler_cls):
    ...
    assert (
        s_deriv.shape == t.shape
    ), f"s_deriv: expected shape {t.shape}, got {s_deriv.shape}"
...
def test_c_skip(scheduler_cls):
    ...
    assert (
        c_skip.shape == t.shape
    ), f"c_skip: expected shape {t.shape}, got {c_skip.shape}"
...
def test_c_noise(scheduler_cls):
    ...
    assert (
        c_noise.shape == t.shape
    ), f"c_noise: expected shape {t.shape}, got {c_noise.shape}"
...
def test_sample_sigma(scheduler_cls):
    ...
    assert (
        sigma.shape == shape_
    ), f"sample_sigma: expected shape {shape_}, got {sigma.shape}"
...
def test_sample_noise(scheduler_cls):
    ...
    assert (
        noise.shape == shape_
    ), f"sample_noise: expected shape {shape_}, got {noise.shape}, sigma shape: {sigma.shape}"
...
def test_sample_prior(scheduler_cls):
    ...
    assert (
        prior.shape == shape_
    ), f"sample_prior: expected shape {shape_}, got {prior.shape}"
...
def test_loss_weight(scheduler_cls):
    ...
    assert (
        weight.shape == sigma.shape
    ), f"loss_weight: expected shape {sigma.shape}, got {weight.shape}"
...
def test_f(scheduler_cls):
    ...
    assert (
        f_out.shape == x.shape
    ), f"f: expected shape {x.shape}, got {f_out.shape}, t shape: {t.shape}"
...
def test_denoise(scheduler_cls):
    ...
    assert (
        denoised.shape == x.shape
    ), f"denoise: expected shape {x.shape}, got {denoised.shape}, sigma shape: {sigma.shape}"
...
def test_loss_fn(scheduler_cls):
    ...
    condition_mask = jnp.ones_like(x, dtype=bool)
    loss = loss_fn(dummy_model, batch, condition_mask)
    assert loss.shape == (), f"loss_fn: expected shape (), got {loss.shape}"
