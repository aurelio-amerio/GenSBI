...
def get_params(id_embedding_kind="absolute", param_dtype=jnp.float32):
	return Flux1JointParams(
		...
		dim_joint=4,
		theta=16,
		id_embedding_kind=id_embedding_kind,
		guidance_embed=False,
		param_dtype=param_dtype
	)

def init_test_model_joint_rope():
	params = get_params(id_embedding_kind="rope", param_dtype=jnp.bfloat16)
	model = Flux1Joint(params)
	return model

def init_test_model_joint_absolute():
	params = get_params(id_embedding_kind="absolute", param_dtype=jnp.bfloat16)
	model = Flux1Joint(params)
	return model
...
def test_flux1joint_rope_warning():
	with pytest.warns(UserWarning, match="Using RoPE embedding for joint density estimation is not recommended. Consider using 'absolute' embeddings instead."):
