...
class BaseSDE(abc.ABC):
    ...
    def get_output_scale_fn(self, scale_min=None):
        if scale_min is None:
            assert (
                not self.scale_min is None
            ), "scale_min must be implemented in the SDE class or provided as an argument"
        ...
        def output_scale_fn(x, t):
            scale = jnp.clip(
                jnp.sqrt(
                    jnp.sum(
                        self.marginal_variance(t[..., None]) * jnp.ones(x.shape), -1
                    )
                ),
                scale_min,
            )
...
class ReverseSDE(abc.ABC):
    ...
    def drift(self, x, t, condition_mask=0, replace_conditioned=True, score_args={}):
        ...
        res = forward_drift(x, t) - diffusion(x, t) ** 2 * self.score(
            jnp.atleast_2d(x), t, **score_args
        )
    ...
    def sample(
        ...
        def diff(t, y, args):
            return jnp.diag(
                jnp.broadcast_to(
                    self.diffusion(y, t, condition_mask, replace_conditioned),
                    (self.dim,),
                )
            )
        ...
        def drift(t, y, args):
            return self.drift(
                y, t, condition_mask, replace_conditioned, score_args=score_args
            )
