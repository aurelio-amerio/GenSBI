...
class BaseSDE(abc.ABC):
    ...
    def f_u(self, x: Array, u: Array) -> Array:
        r"""
        Drift term for the forward diffusion process.

        Computes the drift term :math:`f(x, u) = x \frac{ds}{du} / s(u)` as used in the SDE formulation.
        Where $u \in [0, 1]$.
    ...
    def g_u(self, x: Array, u: Array) -> Array:
        r"""
        Diffusion term for the forward diffusion process.

        Computes the diffusion term :math:`g(x, u) = s(u) \sqrt{2 \frac{d\sigma}{du} \sigma(u)}` as used in the SDE formulation.
        Where $u \in [0, 1]$.
    ...
    def get_score_function(self, F: Callable) -> Callable:
        r"""
        Returns the score function :math:`\nabla_x \log p(x; \sigma)` as described in the EDM paper (eq. 69).
...
class VPEdmScheduler(BaseSDE):
    ...
    def f(self, x, u):
        ...
        return -x * 0.5 * (self.beta_min + self.beta_d * u)

    def g(self, x, u):
        # g(sigma) in the SDE, also known as diffusion term for the forward diffusion process
        return jnp.sqrt(self.beta_min + self.beta_d * u)
...
class VEEdmScheduler(BaseSDE):
    ...
    def time_schedule(self, u):
        # this gives us sigma2(u), in the Song definition of http://arxiv.org/abs/2011.13456 , eq. 31
        # in our notation, t(u) = sigma2(u)
        return self.sigma_min**2 * (self.sigma_max / self.sigma_min) ** (2 * u)

    def sigma(self, t):
        # t = sigma2(u), thus sigma=sqrt(t)
        return jnp.sqrt(t)
    ...
    def sigma_deriv(self, t):
        # dsigma/du = (dsigma/dt * dt/du) evaluated at t0
        return jnp.sqrt(t) * jnp.log(self.sigma_max / self.sigma_min)
    ...
    def f_u(self, x, u):
        # f(x, u) in the SDE, also known as drift term for the forward diffusion process
        return jnp.zeros_like(x)

    def g_u(self, x, u):
        # g(u) in the SDE, also known as diffusion term for the forward diffusion process
        sigma_d = self.sigma_max / self.sigma_min
        return self.sigma_min * sigma_d**u * jnp.sqrt(2 * jnp.log(sigma_d))
